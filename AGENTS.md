...
- **下一步**: 1) K总开新会话派生F004 coder(启动提示词: docs/handbook/launch-prompts/f004-coder-launch.md; ControllerSpec: docs/handbook/controller-specs/f004-coder.md, 12项验收标准含裁决①api-spec回写与裁决③convention-to-rule-mapping同步两项; journal 39预留coder/40预留审查; 设计文档已Approved为唯一设计权威); 2) coder报告→L1流程验收(仅四类行)→test-reviewer独立审查(journal 40, 正式feature无豁免); 3) 后续F005/F007/F012/F013按current-sprint.md排序(current-sprint.md已置sprint2-active)。环境事实: 各会话沙箱环境漂移, uv sync网络受限时卡死(根因与替代构建法见pitfalls.md P009); UV_DEFAULT_INDEX残留时uv run会重写已提交lock(P010, 防护UV_FROZEN=1); 平台hookspath自动stage含untracked文件(P011, 实证4-6: 提交前git diff --cached核对+验收锚定diff范围而非HEAD, F014批次三条新实证已沉淀)环境事实: 各会话沙箱环境漂移, uv sync网络受限时卡死(根因与替代构建法见pitfalls.md P009); UV_DEFAULT_INDEX残留时uv run会重写已提交lock(P010, 防护UV_FROZEN=1); 平台hookspath自动stage(P011, 提交前git diff --cached核对, F003审查产出a577463重复提交实证)
