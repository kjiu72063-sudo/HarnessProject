...
### section 21: 审计收敛后的战略讨论与交接

**背景**: 第十二轮审计确认约束层闭合（零新发现）后，讨论下一步推进方向，发现流程缺陷，并完成项目交接准备。

**21.1 原型缺口发现**

讨论"下一步应该做什么"时，对照 PDF Harness 流程的阶段 1 发现：
...
**结论**: 原型图未开发，PDF 阶段1"原型确认(菱形)"门控未通过。没有原型就做功能拆分或写前端代码，违反 PDF 结构化执行。正确顺序：原型图开发 → 原型确认 → 功能拆分 → 编码实现。

**21.2 上下文环境担忧与两阶段模型**

12 轮审计在同一会话中进行，上下文堆积导致风险：压缩失真、状态漂移、检索噪声、有效性幻觉。

**决策**: 采纳 PDF 两阶段模型 — Init Agent（当前会话）完成初始化+约束层后结束；后续开发在新会话中从持久化记忆冷启动。编码 Agent 5 步标准流程第一步就是读取 AGENTS.md + progress.txt + feature_list.json，不依赖对话上下文。

**21.3 角色分解**

当前 Agent 承担了所有角色（组织者+开发者+架构师+需求分析）。后续应分解：
- **项目组织者**: 读取持久化记忆 → 理解当前状态 → 规划任务 → 委派给编码 Agent
- **编码 Agent**: 接收明确任务 → 实现代码 → 验证 → 更新进度

这与 PDF「Agent 专业化」组件对齐（F004）。

**21.4 交接产物**

1. AGENTS.md 新增「当前阶段与下一步」段 — 新会话冷启动入口
2. progress.txt 补齐至 60 条记录 — 完整可追溯
3. 新增 `docs/handbook/orchestrator-prompt.md` — 项目组织者提示词，含：
   - 角色定义（只规划不写代码）
   - 知识获取路径（6 条：AGENTS.md → conventions → architecture → feature_list → progress → journal）
   - 当前任务序列（5 个 Task：原型开发→功能拆分→后端核心→前端实现→集成验证）
   - 委派格式（输入/输出/验收标准/约束）
   - 完成回调（更新 progress.txt + feature_list.json）

**21.5 持久化记忆完整性验证**

模拟新会话冷启动：仅凭 AGENTS.md → progress.txt → feature_list.json → orchestrator-prompt.md 即可重建完整项目认知。确认所有 12 轮审计的决策、修复、规则均有持久化记录。
