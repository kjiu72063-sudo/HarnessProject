...
test.describe('RequirementPage', () => {
  ...
  test('R1: 完整提交流程 → 页面转至流程监控', async ({ page }) => {
    ...
    await page.fill('textarea[placeholder*="需求"]', '构建一个待办事项应用，支持增删改查')
  ...
  test('R2: 空提交拦截 → 提交按钮禁用', async ({ page }) => {
    ...
    await expect(submitBtn).toBeDisabled()
    await page.fill('textarea[placeholder*="需求"]', 'has-requirement')
    await expect(submitBtn).toBeEnabled()
  })
  ...
  test('R3: 最近项目列表渲染', async ({ page, navigateTo }) => {
    ...
    await page.reload()
    await expect(page.getByText('近期生成')).toBeVisible()
