...
L3 产出回来后，你按以下检查清单验收。**注意：L1 验收只做流程检查，不做内容质量判定。内容质量校验必须委派 L3 设计校验 Agent。**
...
- L1 不得以 L1 验收代替 L3 校验。L1 验收通过后，必须委派 L3 设计校验 Agent 做内容质量校验。
- 修订后的文档必须重新经过 L3 校验 Agent 审阅，L1 不得以"针对性修改""非结构性改动""只是文本增补"等理由跳过校验。

L1 验收通过后的下一步是委派 L3 校验（不是直接推进状态）。L3 校验通过后，L1 才能推进状态（如 Status → Approved）。
